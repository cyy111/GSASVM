...
model=libsvmtrain(train_wine_labels,train_wine,cmd); % SVM模型训练
[~,~,fitness]=libsvmpredict(test_wine_labels,test_wine,model); % SVM模型预测及其精度
%f=1-fitness(1)/100; % 以分类预测错误率作为优化的目标函数值
f=fitness(2); % 以平均均方误差MSE作为优化的目标函数值